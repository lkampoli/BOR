        ...
        if (max_iter==0) | (max_iter==5000):
            ...
            for var in post_vars:
                ...
                for pos in x_positions:
                    ...
                    if (var == 'R'):
                        var_data = np.append(var_data, data[:,var_dict[var]] * (4.1*4.1/(2.2*2.2)) / (0.186*0.186)) #(4.1*4.1) / (0.186*0.186))
                    else:
                        var_data = np.append(var_data, data[:,var_dict[var]] / 2.2)

                save_file = 'WAKE_' + var.upper() + 'xy1.edf.gz'
...
if (sympy_err + sim_err) > 0:
    ...
    for var in post_vars:
        save_file = 'WAKE_' + var.upper() + 'xy1.edf.gz'
        np.savetxt("./output/" + save_file, [np.nan]*data_length) # 865
    save_file = 'WAKE_Vxy1.edf.gz'
    np.savetxt("./output/" + save_file, [np.nan]*data_length) # 865
