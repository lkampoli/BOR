...
class Model(object):
    ...
    def train(self):
        ...
        if self.restart != 1:
            ...
            cost_fun1 = "sum(1.0/DATALEN/1.0*(CENTER_Uxy1-CENTER_U0)**2)" # Mean axial velocity 
            cost_fun2 = "sum(1.0/DATALEN/1.0*(CENTER_Vxy1-CENTER_V0)**2)" # Mean transversal velocity 
            cost_fun3 = "sum(1.0/DATALEN/1.0*(CENTER_Rxy1-CENTER_Rxy0)**2)" # Reynolds Rxy stresses 
            ...
            if self.restart != 2:
                ...
                # Define symbols
                # Frozens
                U0sym = ea.EvalSymbol("CENTER_U0", 0)
                V0sym = ea.EvalSymbol("CENTER_V0", 0)
                R0sym = ea.EvalSymbol("CENTER_Rxy0", 0)

                # Actives
                U1sym = ea.EvalSymbol("CENTER_Uxy1", 0)
                V1sym = ea.EvalSymbol("CENTER_Vxy1", 0)
                R1sym = ea.EvalSymbol("CENTER_Rxy1", 0)

                # Parameters
                I01sym = ea.EvalSymbol("I01", 0)
                I02sym = ea.EvalSymbol("I02", 0)
                
                #Q01sym = ea.EvalSymbol("Q01", 0)
                #Q02sym = ea.EvalSymbol("Q02", 0)
                #Q03sym = ea.EvalSymbol("Q03", 0)
                #Q04sym = ea.EvalSymbol("Q04", 0)
                #Q05sym = ea.EvalSymbol("Q05", 0)
                #Q06sym = ea.EvalSymbol("Q06", 0)
                #Q07sym = ea.EvalSymbol("Q07", 0)
                #Q08sym = ea.EvalSymbol("Q08", 0)
                #Q09sym = ea.EvalSymbol("Q09", 0)
                ...
                #terms_exp1 = [I01sym, I02sym, Q01sym, Q02sym, Q03sym, Q04sym, Q05sym, Q06sym, Q07sym, Q08sym, Q09sym, Q10sym, one, two, mone] + rnc_syms
                #terms_exp1 = [I01sym, I02sym, Q01sym, Q02sym, Q03sym, Q04sym, Q05sym, Q06sym, Q07sym, Q08sym, Q09sym, one, two, mone] + rnc_syms

                # Arity 0 weights
                term_exp1_weights = list(0.6/2*np.ones(2)) + \
                                    list(0.2/3*np.ones(3)) + \
                                    list(0.2/num_rnc*np.ones(num_rnc))
                #term_exp1_weights = list(0.6/11*np.ones(11)) + \
                #                    list(0.2/3*np.ones(3)) + \
                #                    list(0.2/num_rnc*np.ones(num_rnc))
                ...
                # Symbols to load
                frozens = [U0sym, V0sym, R0sym]
                actives = [U1sym, V1sym, R1sym]
