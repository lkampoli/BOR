...
for j in range(len(profiles_wake)):
    try:
       # RANS Data
       y_num, y_U_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/"+profiles_wake[j]+"_U.xy", unpack=True, usecols=[0,1])
       y_U_num = y_U_num / 2.2

       # Experimental Data
       y_exp, y_U_exp = np.loadtxt("../../ref_data/postProcessing/sampleDict/0/"+profiles_wake[j]+"_U.xy", unpack=True, usecols=[1,3])
       ...
       err_U_wake = mean_absolute_percentage_error(y_U_exp, y_U_num_int)
       if (err_U_wake < 0):
	       err_U_wake = 9999
    except:
       err_U_wake = 9999
...
## Mean transversal velocity in the WAKE
########################################
#for j in range(len(profiles_wake)):
#    try:
#       # RANS Data
#       y_num, y_V_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/"+profiles_wake[j]+"_U.xy", unpack=True, usecols=[0,2])
#       y_V_num = y_V_num / 2.2
#
#       # Experimental Data
#       y_exp, y_V_exp = np.loadtxt("../../ref_data/postProcessing/sampleDict/0/"+profiles_wake[j]+"_U.xy", unpack=True, usecols=[1,4])
#
#       # interpolate experimental onto RANS data
#       g = interp1d(y_num, y_V_num)
#       y_V_num_int = g(y_exp)
#
#       # limiter to avoid negative values
#       err_V_wake = mean_absolute_percentage_error(y_V_exp, y_V_num_int)
#       if (err_V_wake < 0):
#           err_V_wake = 9999
#    except:
#       err_V_wake = 9999
#
#    print("err_V_wake: ",err_V_wake)
#
## ===============================================================================
...
for j in range(len(profiles_wake)):
    try:
       ...
       y_num, y_R_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/"+profiles_wake[j]+"_Rall.xy", unpack=True, usecols=[0,2])
...
#err_bor = err_U_wake + err_V_wake + err_R_wake
err_bor = err_U_wake + err_R_wake
print("err_bor =", err_bor)

# write the value of cost functions in output folder
cost_list = [err_U_wake, err_R_wake]
#cost_list = [err_U_wake, err_V_wake, err_R_wake]
