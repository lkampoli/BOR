...
# Mean transversal velocity in the BOW
######################################
for j in range(len(profiles_bow)):
    try:
       # RANS Data
       y_num, y_V_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/"+profiles_bow[j]+"_U.xy", unpack=True, usecols=[0,2])
       y_V_num = y_V_num / 2.2

       # Experimental Data
       y_exp, y_V_exp = np.loadtxt("../../ref_data/postProcessing/sampleDict/0/"+profiles_bow[j]+"_U.xy", unpack=True, usecols=[1,4])
       ...
       # limiter to avoid negative values
       err_V_bow = mean_absolute_percentage_error(y_V_exp, y_V_num_int)
       if (err_V_bow < 0):
	       err_V_bow = 9999
    except:
       err_V_bow = 9999
    
    print("err_V_bow: ",err_V_bow)
...
for j in range(len(profiles_bow)):
    try:
       ...
       y_num, y_R_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/"+profiles_bow[j]+"_Rall.xy", unpack=True, usecols=[0,2])
...
#try:
#   # RANS Data
#   xRm, xRRm = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/frozen_BOW_R.xy", unpack=True, usecols=[0,4])
#   xRRm = xRRm *(4.1*4.1)/(0.186*0.186)
#
#   # Experimental Data
#   xEm, xREm = np.loadtxt("../../ref_data/postProcessing/sampleDict/0/frozen_Rwall_BOW.xy", unpack=True, usecols=[0,2])
#
#   g = interp1d(xRm, xRRm)
#   xRRmint = g(xEm)
#       
...
#   
#   err_shear_bow = err_shear_bow + scipy.integrate.simps(msexRm,xEm)/(max(xEm)-min(xEm))
#   #if (msexRm.any() > 10):
#   if (abs(err_shear_bow) > 10):
#	   err_shear_bow = 9999
#except:
#   err_shear_bow = 9999
#
...
err_bor = err_U_bow + err_V_bow + err_R_bow
print("err_bor =", err_bor)

# write the value of cost functions in output folder
cost_list = [err_U_bow, err_V_bow, err_R_bow]
costs = [[0.001] for i in range(3) for i in range(1)] 
