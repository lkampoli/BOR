...
#    try:
#       # RANS Data
#       yRm, yRRm = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/"+profiles_bow[j]+"_R.xy", unpack=True, usecols=[0,2])
#       yRRm = yRRm *(4.1*4.1)/(0.186*0.186)
# 
#       # Experimental Data
#       yEm, yREm = np.loadtxt("../../ref_data/postProcessing/sampleDict/0/"+profiles_bow[j]+"_U.xy", unpack=True, usecols=[1,7])
#
#       g = interp1d(yRm, yRRm)
#       yRRmint = g(yEm)
#      
...
#       
#       err_shear_bow = err_shear_bow + scipy.integrate.simps(mseyRm,yEm)/(max(yEm)-min(yEm))
#    except:
#       err_shear_bow = 9999
#
# *******************************************************************************
...
cost_list = [err_U_bow]
costs = [[0.001] for i in range(1) for i in range(1)] # change the first range(2)
