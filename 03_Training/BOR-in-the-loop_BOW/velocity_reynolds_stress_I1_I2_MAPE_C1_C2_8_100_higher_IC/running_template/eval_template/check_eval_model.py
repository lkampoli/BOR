...
for j in range(len(profiles_bow)):
    try:
       ...
       y_num, y_U_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/0/"+profiles_bow[j]+"_U.xy", unpack=True, usecols=[0,1])
       y_U_num = y_U_num/2.2
       print(y_U_num)
...
for j in range(len(profiles_bow)):
    try:
       ...
       y_num, y_R_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/0/"+profiles_bow[j]+"_R.xy", unpack=True, usecols=[0,2])
