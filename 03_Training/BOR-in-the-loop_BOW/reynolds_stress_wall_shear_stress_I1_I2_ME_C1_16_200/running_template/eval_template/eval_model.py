...
bor.fix_R_2000()
#subprocess.call(["sed", "-i", 's/mapField/mapField       planarInterpolation/g', case_dir+"/2000/R"])
...
for j in range(len(profiles_bow)):
    try:
       ...
       err_R_bow = max_error(y_R_exp, y_R_num_int)
       if (err_R_bow < 0):
	       err_R_bow = 9999
    except:
       err_R_bow = 9999
...
try:
   ...
   # limiter to avoid negative values
   err_shear_bow = max_error(x_R_exp, x_R_num_int)
   if (err_shear_bow < 0):
	   err_shear_bow = 9999
except:
   err_shear_bow = 9999
...
err_bor = err_R_bow + err_shear_bow
