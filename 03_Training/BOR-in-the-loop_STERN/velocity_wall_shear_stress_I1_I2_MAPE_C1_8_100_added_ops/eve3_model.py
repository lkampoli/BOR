    ...
    def train(self):
        ...
        if self.restart != 1:
            ...
            if self.restart != 2:
                ...
                divide= ea.Symbol("/", 2)
                exp   = ea.Symbol("np.exp", 1, nice_name="exp")
                log   = ea.Symbol("we.aux.log", 1, nice_name="log")
                nexp  = ea.Symbol("we.aux.nexp", 1, nice_name="nexp") # negative exponential
                tanh  = ea.Symbol("np.tanh", 1, nice_name="tanh")  # tanh
                power = ea.Symbol("np.pow", 2, nice_name="**")
                sqr   = ea.Symbol("np.square", 1, nice_name="sqr")
                sqrt  = ea.Symbol("we.aux.sqrt", 1, nice_name="sqrt")
                ...
                # Arity 0 symbols
                terms_exp1_ar0 = [I1sym, I2sym, one, two, mone] + rnc_syms
                
                # Arity 1 symbols
                terms_exp1_ar1 = [exp, log, nexp, tanh, sqr]
                
                # Arity 2 symbols
                terms_exp1_ar2 = [times, plus, minus]
                
                # All symbols (separated with respect to arity)
                #syms_exp1 = [terms_exp1, [], [times, plus, minus]]
                #syms_exp1 = [terms_exp1, [exp, log, nexp, tanh, sqr], [times, plus, minus]]
                #syms_exp1 = [terms_exp1, [exp, log, nexp, tanh, sqr], [times, plus, minus, divide, power]]
                #syms_exp1 = [terms_exp1, [], [times, plus, minus, divide, power]]
                syms_exp1 = [terms_exp1_ar0, terms_exp1_ar1, terms_exp1_ar2]
                print(len(syms_exp1))
                ...
                sym_ar0_weights = term_exp1_weights
                sym_ar1_weights = [1./2, 1./2, 1./2, 1./2, 1./2]
                sym_ar2_weights = [1./3, 1./3, 1./3]

                # Probabilities and weights for different arities
                pterms = [0.4]*nexpr # arity 0
                pones  = [0.2]*nexpr # arity 1
                ptwos  = [0.4]*nexpr # arity 2
                psyms  = [pterms, pones, ptwos]
                
                # All weights
                #sym_exp1_weights = [term_exp1_weights, [], sym_ar2_weights]
                #sym_exp1_weights = [term_exp1_weights, [], sym_ar2_weights]
                sym_exp1_weights = [sym_ar0_weights, sym_ar1_weights, sym_ar2_weights]
                print(len(sym_exp1_weights))
                ...
                # DG algorithm
                    ...
                    factory = ea.CFactory(
                        ...
                        ptwo=ptwos,                                         # probability of selecting arity 2 symbol
