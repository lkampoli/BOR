...
for j in range(len(profiles_stern)):
    try:
       # RANS Data
       y_num, y_U_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/"+profiles_stern[j]+"_U.xy", unpack=True, usecols=[0,1])
 
       # Experimental Data
       y_exp, y_U_exp = np.loadtxt("../../ref_data/postProcessing/sampleDict/0/"+profiles_stern[j]+"_U.xy", unpack=True, usecols=[1,3])
...
err_bor = err_U_stern + err_R_stern + err_shear_stern
print("err_bor =", err_bor)

# write the value of cost functions in output folder
cost_list = [err_U_stern, err_R_stern, err_shear_stern]
