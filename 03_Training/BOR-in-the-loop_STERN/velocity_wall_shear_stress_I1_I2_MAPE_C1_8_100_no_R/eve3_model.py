"""EVE3 training on the medium BOR testcase adding R term in the loop."""
...
data_path = "./ref_data/postProcessing/sampleDict/0"

# Path to running folder for external evaluation
ext_eval_path = "./running_template/eval_template"
...
class Model(object):
    ...
    def train(self):
        ...
        if self.restart != 1:
            ...
            cost_fun1 = "C1" # cost function of velocity in the bow region
            #cost_fun2 = "C2" # cost function of velocity in the central region
            #cost_fun3 = "C3" # cost function of velocity in the stern region
            #cost_fun4 = "C4" # cost function of velocity in the wake region
            #cost_fun5 = "C5" # cost function of shear stress in the bow region
            #cost_fun6 = "C6" # cost function of shear stress in the central region
            #cost_fun7 = "C7" # cost function of shear stress in the stern region
            #cost_fun8 = "C8" # cost function of shear stress in the wake region
            #cost_fun = [cost_fun1, cost_fun2, cost_fun3, cost_fun4, cost_fun5, cost_fun6, cost_fun7, cost_fun8] # also put it in the list
            cost_fun = [cost_fun1]
            ...
            if self.restart != 2:
                ...
                # Actives
                C1sym = ea.EvalSymbol("C1", 0) # also add other cost function here
                #C2sym = ea.EvalSymbol("C2", 0) # also add other cost function here
                #C3sym = ea.EvalSymbol("C3", 0) # also add other cost function here
                #C4sym = ea.EvalSymbol("C4", 0) # also add other cost function here
                #C5sym = ea.EvalSymbol("C5", 0) # also add other cost function here
                #C6sym = ea.EvalSymbol("C6", 0) # also add other cost function here
                #C7sym = ea.EvalSymbol("C7", 0) # also add other cost function here
                #C8sym = ea.EvalSymbol("C8", 0) # also add other cost function here

                # Parameters
                I1sym = ea.EvalSymbol("I1", 0)
                I2sym = ea.EvalSymbol("I2", 0)
                #ksym  = ea.EvalSymbol("k" , 0) # added feature
                #Q2sym = ea.EvalSymbol("Q2", 0) # added feature
                #Q4sym = ea.EvalSymbol("Q4", 0) # added feature
                #Q5sym = ea.EvalSymbol("Q5", 0) # added feature
                #Q7sym = ea.EvalSymbol("Q7", 0) # added feature
                #Q8sym = ea.EvalSymbol("Q8", 0) # added feature
                #Q9sym = ea.EvalSymbol("Q9", 0) # added feature
                ...
                # Arity 0 symbols
                terms_exp1 = [I1sym, I2sym, one, two, mone] + rnc_syms
                #terms_exp1 = [I1sym, I2sym, ksym, one, two, mone] + rnc_syms
                #terms_exp1 = [I1sym, I2sym, ksym, Q2sym, Q4sym, Q5sym, Q7sym, Q8sym, Q9sym, one, two, mone] + rnc_syms
                #terms_exp1 = [I1sym, I2sym, ksym, Q2sym, Q4sym, Q5sym, Q7sym, Q9sym, one, two, mone] + rnc_syms

                # Arity 0 weights
                #term_exp1_weights = list(0.5/3*np.ones(3)) + \
                term_exp1_weights = list(0.5/2*np.ones(2)) + \
                                    list(0.2/3*np.ones(3)) + \
                                    list(0.3/num_rnc*np.ones(num_rnc))
                ...
                # Symbols to load
                actives = [C1sym]
                #actives = [C1sym, C2sym, C3sym, C4sym, C5sym, C6sym, C7sym, C8sym] # also put it in the actives list

                # Additional kwargs dict for iFndi creation
                indi_kwargs_exp1 = {}
        ...
        for gen in range(start_gen, self.ngens+1):
            ...
            # Genetic operations including evolutionary pressure
