...
for j in range(len(profiles_center)):
    try:
       # RANS Data
       y_num, y_U_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/"+profiles_center[j]+"_U.xy", unpack=True, usecols=[0,1])
       y_U_num = y_U_num / 2.2

       # Experimental Data
       y_exp, y_U_exp = np.loadtxt("../../ref_data/postProcessing/sampleDict/0/"+profiles_center[j]+"_U.xy", unpack=True, usecols=[1,3])
       ...
       err_U_center = mean_absolute_percentage_error(y_U_exp, y_U_num_int)
       if (err_U_center < 0):
	       err_U_center = 9999
    except:
       err_U_center = 9999
...
for j in range(len(profiles_center)):
    try:
       ...
       y_num, y_R_num = np.loadtxt(case_dir+"/postProcessing/sampleDict/2000/"+profiles_center[j]+"_Rall.xy", unpack=True, usecols=[0,2])
