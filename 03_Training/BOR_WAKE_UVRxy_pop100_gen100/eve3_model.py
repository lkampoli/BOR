...
class Model(object):
    ...
    def train(self):
        ...
        if self.restart != 1:
            ...
            cost_fun1 = "sum(1.0/DATALEN/1.0*(WAKE_Uxy1-WAKE_U0)**2)" # Mean axial velocity 
            cost_fun2 = "sum(1.0/DATALEN/1.0*(WAKE_Vxy1-WAKE_V0)**2)" # Mean transversal velocity 
            cost_fun3 = "sum(1.0/DATALEN/1.0*(WAKE_Rxy1-WAKE_Rxy0)**2)" # Reynolds Rxy stresses 
            ...
            if self.restart != 2:
                ...
                # Define symbols
                # Frozens
                U0sym = ea.EvalSymbol("WAKE_U0", 0)
                V0sym = ea.EvalSymbol("WAKE_V0", 0)
                R0sym = ea.EvalSymbol("WAKE_Rxy0", 0)

                # Actives
                U1sym = ea.EvalSymbol("WAKE_Uxy1", 0)
                V1sym = ea.EvalSymbol("WAKE_Vxy1", 0)
                R1sym = ea.EvalSymbol("WAKE_Rxy1", 0)
